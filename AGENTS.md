...
- **Release 资产命名（两层名字，勿再混淆）**：GitHub 服务端会静默吃掉资产 **slug**（`name`，进下载 URL）里的非 ASCII 字符——连续一段折叠成一个 `.`、开头的整段删除，所以 `极思G界址点互转工具_4.4.0_x64-setup.exe` → `G._4.4.0_x64-setup.exe`，中文 slug 物理上不可能（换上传工具/换 runner 平台都无用）。但 Release 列表**显示**的是 `label`：`tauri-action` 会自动把 label 设成中文原名（页面看到的仍是中文名），而 `gh release upload` 不设 label，必须补一次 `gh api -X PATCH repos/.../releases/assets/{id} --input label.json`（label 由 node 从 `tauri.conf.json` 读 `productName` 生成，勿把中文经 shell 参数传），否则只有便携版露出 `G._*`（v4.4.0 就踩了）。推论：`latest.json` 的 url 必须取 API 的 `browser_download_url`，绝不能自己拼中文名（v4.4.0 拼了 → 404 → 老客户端更新下载失败）。核对资产用 `gh api repos/.../releases/tags/<tag> -q '.assets[] | "\(.name) | \(.label)"'`。暂存目录 `其他相关tbx放进去release/`（gitignored）放本地未签名包，可用中文名。
