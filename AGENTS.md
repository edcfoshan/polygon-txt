...
- **Release 资产命名（两层名字 + 一个规范化脚本）**：GitHub 每个资产有 `name`（slug，决定下载 URL 与 `Content-Disposition` 落盘文件名）和 `label`（Release 列表显示名）。服务端会把 slug 里的非 ASCII 静默折叠（`极思G界址点互转工具_4.4.0_x64-setup.exe` → `G._4.4.0_x64-setup.exe`：连续一段折成 `.`、开头整段删除），中文 slug 物理不可能（换上传工具/换 runner 平台都没用）。方案：tauri-action 上传后跑 `scripts/normalize-release-assets.mjs`（CI 的 `Normalize asset names` 步骤；本地手工发版同一条命令）把 **slug 改成品牌 ASCII `JisigG_{ver}_*`、label 保留中文原名**——只 PATCH 元数据、不重传，`.sig` 基于 exe 字节故不受影响。不做的后果：用户存盘拿到 `G._4.4.0_x64-setup.exe`。另两条铁律：`latest.json` 的 url 必须取 normalize 之后 API 的 `browser_download_url`，绝不能拼中文名（v4.4.0 拼过 → 404 → 老客户端更新下载失败）；核对资产只信 `gh api repos/.../releases/tags/<tag> -q '.assets[] | "\(.name) | \(.label)"'`。本地暂存目录 `其他相关tbx放进去release/`（gitignored）可用中文名。
