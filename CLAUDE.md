...
- **v4.3 UI 重排（页签式，取代 v3.0 折叠排布）**：左栏 = ①导入数据 + ②输出与选项（两卡**固定展开**，acc-h 无 onclick/箭头）；中栏 = 页签切换**投影 / 表头 / 字段 / 界址点**（`.mid-tabs`/`.mid-pane`，默认激活「字段」；界址点页签即部备案TXT 设置，`#bbTag` 徽标在 pane 标题行）；右栏预览不变。`.accsw`（高级开关）在 `.acc-h` 作用域外需用 `.mid-pane .accsw` 补样式；左栏窄列时 `.pnl-l .mode-seg` 换行防裁字。历史教训：手改中栏 HTML 曾多写一个 `</div>` 使浏览器提前闭合 `.main`、右栏 `.ctr` 变成 `.main` 兄弟节点掉到底部——改面板结构后务必用 `document.querySelector(".main").children.length === 3` 自检
- **v3.4 右栏三页签（仅面→TXT）**：`swPvTab`（main.js）切「TXT 预览 | 属性表 | 地图」。**属性表**（`src/tableview.js`，手写表格+分页 100/页，0 表格库）+ **地图**（`src/mapview.js`，Leaflet preferCanvas + 天地图 img_w/cia_w XYZ）+ **筛选导出**（`src/querybuilder.js` 结构化条件构建器：条件行 + 行间且/或（按行序左折叠自动加括号）、IN 多选复选弹层、值候选点选、IS NULL；生成 WHERE 文本喂 `src/filterparser.js` 求值）。所见即所得：筛中的行 = 导出范围——前端求值得 `(si,pi)` uid 列表 → `ShpToTxtOptions.plot_filter` → `convert.rs` 在 `match output_mode` 前一处过滤（三模式全覆盖），预览同吃（预览=导出）；`read_plot_table_geo` IPC 回传全量结构化地块（attrs + WGS84 rings），属性表/地图不吃筛选。地图逐源推 CM，**优先级：crs_info 的 cm（直读，不依赖 b）→ z+band → 坐标前缀推断（无元数据兜底，有歧义）**；逆投影前东坐标按数据带号「重贴 million 块」（`e = x − Z_data×1e6 + round(cm/3)×1e6`）：处理「37 带东侧溢出到 38M」（如揭阳 38_057_383 = 37_500_000+557_383，floor(x/1e6) 会误判 38 带）与 6° 带前缀块 ≠ 3° 带网格两种情形；无前缀且无 crs_info → `degraded=true`（`rings:[]`，表格仍可用、地图灰底）。回归测试 `table_geo_zone37_spillover_easting` / `table_geo_band6_zone20`。uid 口径：si = SHP 文件下标 / GDB 产出 sources 下标（仅面状+选中图层），pi = index_in_source。共享态 `TV.getFilterUids()/getSelectedUid()/getFieldNames()/getDistinctValues()`；QB 条件存 `tg_flt2`（JSON rows）；CSS zoom C 区挂 `#panePv/#paneTbl`（**paneMap 不缩放**——zoom 错位 Leaflet 鼠标坐标）；CSP `img-src` 已放行 `https://*.tianditu.gov.cn`；tileerror 8s 内 ≥3 → **先轮换 `TKS[]` 下一个 key 重建瓦片层**（每 key 一轮），全败才灰底降级。**v4.1 增**：属性表「显示字段别名」勾选（`tg_tbl_alias`；GDB 文件真别名 + `BUILTIN_ALIASES` 内置字典兜底，别名经 `read_plot_table_geo` 的 `source_aliases` 回传，gdb.rs `read_table_schema_meta` 读图层字段区 `Field.alias`）；单元格点击弹完整值气泡（截断检测 scrollWidth>clientWidth）；地图「显示界址点」（`tg_map_jpts`）：txt.rs `number_plot_points`（从 generate_txt 拆出的编号纯函数，oj/oc 生效）算标签 → `PlotTablePlot.points` 回传 → 选中地块常驻标注 + zoom≥17 视口标注（裁剪+2000 上限）；地图导航条（⏮◀▶⏭+计数）按筛选命中集循环遍历，经 `TV.setUid(uid,'table')` 三联动。**属性表 = 源字段全集视图**（v4.1 起）：`plot_table_geo` 的 attrs 取 `ImportSource.field_names`（字段区顺序，含 OBJECTID/Shape_*）× `PlotWithSource.attributes`，而非 TXT 输出列——筛选语句字段名/别名切换同口径；**GDB 层 OBJECTID 恢复**（geonative 库把它抽成 Feature.fid 且 schema 剔除——库路径从 `read_table_schema_meta` 的 oid 字段名前置回字段表、`feature.fid` 回填属性；回退路径不再跳过 ObjectId 值），并按 OBJECTID 数值升序 sort + 重排 index_in_source（属性表/导出顺序一致）；同数据刷新不重置地图视图（`state.lastSig` 签名比对，仅新数据集 refit），`#paneMap` 输入不入全局 updatePreview 绑定
...
- `bubeian`（v4.3，`Option<BubeianOptions>`）：部备案模板导出——坐标行 6 列（追加距离、埋桩两列）。`stake_field` 埋桩来源字段（空=不读）、`stake_default` 兜底值、`distance_unit` 米/千米/厘米、`distance_decimals` 小数位。距离在 `txt.rs::point_distances` 按环闭合计算（闭合点 0，开口环连首点），`apply_bubeian_stakes`（convert.rs）在源构建阶段回填 `PlotData.stake`。前端「部备案TXT」卡片独立按钮（`runShpToTxt(true)` 注入 `opt.bubeian`，标准导出不受影响）。测试 `cargo test --test bubeian_txt_test`
...
### GDB 快速导入（v4.1）
`import_gdb` 走 `gdb::read_gdb_catalog`：仅读目录 + 各图层文件头（40B `valid_record_count`=要素数）+ 字段区（64KB 内：字段名/别名/几何类型 code 4=Polygon/`GeomFieldMeta.extent_xy` 范围/srs_wkt），**不解码要素**——大库（几十万要素）导入毫秒级。要素解码延迟到预览/导出：`gdb::read_gdb_filtered(path, only_layers)` 按所选图层跳过未选层的解码（占位 layer_info 保持索引对齐），`collect_import_sources` 的 GDB 分支传 `selected_layers`。`read_gdb(path)` = filtered(None) 保持兼容。地图瓦片 maxNativeZoom=18、maxZoom=20（18 级以上瓦片放大显示）。
