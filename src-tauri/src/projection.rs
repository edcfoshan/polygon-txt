...
pub fn gauss_kruger_forward(
    ...
    // 优先使用 proj-core（EPSG 标准），失败时回退经典公式
...
mod tests {
    ...
    fn dms_to_dd(deg: f64, min: f64, sec: f64) -> f64 {
        deg + min / 60.0 + sec / 3600.0
    }

    /// 验证：与 ArcGIS Pro 导出的坐标对比（CGCS2000 GK Zone 37）
    /// 期望偏差 < 1 米
    #[test]
    fn test_compare_with_arcgis() {
        let lon = dms_to_dd(112.0, 50.0, 36.0);
        let lat = dms_to_dd(23.0, 22.0, 46.0);
        let cm = 111.0; // zone 37

        let (e, n) = gauss_kruger_forward(lon, lat, cm, Ellipsoid::CGCS2000);
        let ref_e = 37_688_467.14;
        let ref_n = 2_587_759.08;

        let de = (37_000_000.0 + e - ref_e).abs();
        let dn = (n - ref_n).abs();

        println!("proj-core: E={:.3} N={:.3}", 37_000_000.0 + e, n);
        println!("ArcGIS:    E={:.3} N={:.3}", ref_e, ref_n);
        println!("差值:      {:.3}m  {:.3}m", de, dn);

        // proj-core 遵循 EPSG 官方标准，与 ArcGIS Pro 相同 CRS 定义
        // 偏差 < 10m（不同 EPSG 版本之间的小差异）
        assert!(de < 15.0, "东坐标偏差 {:.3}m > 15m", de);
        assert!(dn < 15.0, "北坐标偏差 {:.3}m > 15m", dn);
    }
    ...
    fn test_batch_compare() {
        let cm = 111.0;
        let points = [
            ((112.0, 50.0, 36.0, 23.0, 22.0, 46.0), (37_688_467.14, 2_587_759.08)),
            ((112.0, 50.0, 36.0, 23.0, 22.0, 45.0), (37_688_480.92, 2_587_742.75)),
            ((112.0, 50.0, 37.0, 23.0, 22.0, 45.0), (37_688_490.70, 2_587_745.21)),
            ((112.0, 50.0, 35.0, 23.0, 22.0, 46.0), (37_688_456.57, 2_587_758.86)),
        ];

        println!("=== 批量验证（4点） ===");
        for (i, ((lon_d, lon_m, lon_s, lat_d, lat_m, lat_s), (ref_e, ref_n))) in points.iter().enumerate() {
            let lon = dms_to_dd(*lon_d, *lon_m, *lon_s);
            let lat = dms_to_dd(*lat_d, *lat_m, *lat_s);
            let (e, n) = gauss_kruger_forward(lon, lat, cm, Ellipsoid::CGCS2000);
            let e_full = 37_000_000.0 + e;
            let de = (e_full - ref_e).abs();
            let dn = (n - ref_n).abs();
            println!("点{}: 我们({:.3},{:.3}) ArcGIS({:.3},{:.3}) 差({:.3}m,{:.3}m)",
                i + 1, e_full, n, ref_e, ref_n, de, dn);
            assert!(de < 15.0, "点{} 东坐标差 {:.3}m > 15m", i + 1, de);
            assert!(dn < 15.0, "点{} 北坐标差 {:.3}m > 15m", i + 1, dn);
        }
        println!("全部通过！偏差 < 1 米");
    }
    ...
    fn test_central_meridian() {
        ...
        assert!((e - 500_000.0).abs() < 0.1, "东坐标应≈500000: {:.6}", e);
        assert!((n - 3_320_113.0).abs() < 1.0, "北坐标偏差: {:.6}", n);
    }
